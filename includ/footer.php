<div id="footer">

  <div class="footer-top">
    <div class="container">
      <div class="row">

        <div class="col-lg-3 col-md-6 footer-info" id="Localizacao">
           <h4>Localização:</h4>
           <h6>Macapá AP</h6>
           <hr id="localizacao-linha">
          <h5>Horário de funcionmento</h5>
            <h6>De Segunda a Sexta das 08:00 às
             18:00 eSábados das 09:00 às 12:00. </h6>
            
        </div>

        <div class="col-lg-3 col-md-6 footer-links">
          <h4>Categorias</h4>
          <ul>
            <li> <a href="#">Home</a></li>
            <li> <a href="#">produto</a></li>
            <li> <a href="#">Services</a></li>
            <li> <a href="#">cadastre-se</a></li>
            <li><a href="#">pergutas frequentes</a></li>
          </ul>
        </div>
        <div class="col-lg-3 col-md-6 footer-links">
          <h4>Mais imóveis </h4>
          <ul>
            <li> <a href="#">Casas</a></li>
            <li> <a href="#">Apartamentos</a></li>
            <li> <a href="#">Kit net</a></li>
            <li> <a href="#">Terrenos</a></li>
            <li><a href="#">Em condominios</a></li>
          </ul>
        </div>
        <div class="col-lg-3 col-md-6 footer-contact">
          <h4>Contatos</h4>
          <p>
           
            <strong>Telefone:</strong> (96)991082902<br>
            <strong>Email:</strong> casa-facil@gmail.com<br>
          </p>

          <div class="social-links">
            <a href="#" id="whatsapp"> <i  class="fab fa-whatsapp"  ></i></a>
            <a href="#" id="twitter"> <i  class="fab fa-twitter"></i></a>
            <a href="#" id="facebook"><i class="fab fa-facebook-f"></i></a>
            <a href="#" id="youtube"><i class="fab fa-youtube"></i></a>
            <a href="#" id="instagram"><i class="fab fa-instagram"></i></a>
          </div>
        </div>
      </div>
    </div>
  </div>

  <div class="container">
    <div class="copyright">
      &copy;  <strong>Macapá</strong>AP
    </div>
    
 </div>

</div>
<script src="js/jquery-3.2.1.min.js"></script>
  <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery-validate/1.17.0/jquery.validate.min.js"></script>
  <script src="js/popper.min.js"></script>
  <script src="js/bootstrap.min.js"></script>
  <script src="js/main.js"></script>
  <script src="https://code.jquery.com/jquery-3.2.1.slim.min.js" integrity="sha384-KJ3o2DKtIkvYIK3UENzmM7KCkRr/rE9/Qpg6aAZGJwFDMVNA/GpGFF93hXpG5KkN" crossorigin="anonymous"></script>
 <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.12.9/umd/popper.min.js" integrity="sha384-ApNbgh9B+Y1QKtv3Rn7W3mgPxhU9K/ScQsAP7hUibX39j7fakFPskvXusvfa0b4Q" crossorigin="anonymous"></script>

 <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/js/bootstrap.min.js" integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl" crossorigin="anonymous"></script>
</body>
</html>
<!-- Bootstrap core JavaScript -->
